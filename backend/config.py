...
# ---------------------------------------------------------------------------
# Northeast India — CLIMORA Primary Deployment Region
# ---------------------------------------------------------------------------
# All 8 states of India's North Eastern Region
NER_STATES = [
    "Arunachal Pradesh",
    "Assam",
    "Manipur",
...
# NER states that were NOT represented in the V2 synthetic training data.
# Predictions for these states are allowed but clearly labelled:
...
# All states where CLIMORA predictions are supported (NER + V2 training states)
SUPPORTED_STATES_FOR_PREDICTION = list(
    set(NER_STATES) | set(VALID_STATE_REGIONS)
)

# Approximate India bounding box (for fast pre-check before geocoding)
INDIA_BBOX = {
    "lat_min": 6.0,
    "lat_max": 37.5,
    "lon_min": 68.0,
...
# ---------------------------------------------------------------------------
# Environmental Data Service — Cache TTLs (seconds)
# ---------------------------------------------------------------------------
WEATHER_CACHE_TTL_S: float = 900.0    # 15 minutes (matches Open-Meteo update cycle)
TERRAIN_CACHE_TTL_S: float = 86400.0  # 24 hours   (terrain is static)
GEOCODING_CACHE_TTL_S: float = 3600.0 # 1 hour     (reverse-geocode result)
